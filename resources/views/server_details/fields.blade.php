...
$owner_array = ["Avengers","Dragons", "Guardians", "Incredibles", "Justice League", "Seekers", "Transformers", "Performance", "SCE", "Product Owners"];
?>
...
<div class="row">
    ...
    <div class="form-group col-sm-3">
        ...
        <select name="server_owner" class="form-control"  required="required">
        @else
        <select name="server_owner" class="form-control"  disabled>
        @endhasanyrole
            <option value="">Select .... </option>
            @foreach($owner_array as $owa)
                @if($this_is_edit)
                <option value="{{ $owa }}" @if($owa==$record->server_owner) selected='selected' @endif >{{ $owa }}</option>
                @else
