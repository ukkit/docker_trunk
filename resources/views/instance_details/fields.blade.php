...
// $jenkins_token_array = [["US Jenknis Server (andcsv-svgcjk01)","11d0c46e4f38bf0a2a4ea82ef9d5266294"],["Pune Jenkins Server 1 (10.192.110.23)","f92871a081a7d7ec3353d538bdac9b5f"],["Pune Jenkins Server 2 (ppumsv-svgjk01d)","113b0a8f28346617b9065b1677a240e14d"],["Flyboy Home Server","11331b2b5c82107a5d499ea6500034ece1"]];
...
?>
    ...
    <div class="row">
        ...
        {{-- JENKINS REDENTIALS --}}
        <div class="form-group col-sm-2">
            {!! Form::label('jenkins_credentials_id', 'Jenkins Token:') !!}
            <select name="jenkins_credentials_id" class="form-control select-jenkins-credentials-id">
                <option value="">Select .... </option>
                @foreach($jenkins_credentials as $jcd)
                    @if($this_is_edit)
                        <option value="{{ $jcd->id }}" @if($jcd->id==$record->jenkins_credentials_id) selected='selected' @endif >{{ $jcd->server_name_ip }} {{ $jcd->note }}</option>
                    @else
                        <option value="{{ $jcd->id }}"> {{ $jcd->server_name_ip }} {{ $jcd->note }}</option>
                    @endif
                @endforeach
            </select>
        </div>
