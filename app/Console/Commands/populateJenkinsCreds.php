...
class populateJenkinsCreds extends Command
{
    ...
    public function handle()
    {
        ...
        if (!one_time_execution($scriptID)) {
            $clean_inst = DB::table('instance_details')->whereNotNull('jenkins_credentials_id')->get();
            foreach ($clean_inst as $ci) {
                DB::table('instance_details')->where('id', $ci->id)->update(['jenkins_credentials_id' => null]);
            }
            DB::table('jenkins_credentials')->delete();
            ...
            foreach ($unique_arr as $value) {
                ...
                echo $expl[0] . " | " . $insertedid . " ID added \n";
            }

            $all_jencred = DB::table('jenkins_credentials')->select('id', 'server_name_ip', 'jenkins_token')->whereNull('deleted_at')->get();
            echo " --->" . count($all_jencred) . "\n";
            $arr = json_decode(json_encode($all_jencred), true);
            echo " ===>" . count($arr) . "\n";

            echo "Starting for each \n";
